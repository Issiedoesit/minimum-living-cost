...
const ReviewCards = (props) => {
  ...
  return (
   <section className='inter min-w-full px-10 lg:pl-28 items-center gap-5 lg:gap-10 h-fit grid grid-cols-1 lg:grid-cols-2'>
        <section className='py-28 space-y-7 col-span-1'>
            <div className='relative lg:text-left'>
                <h1 className='inter-400 sm:text-xl'>{props.review}</h1>
                <img src={Quotes} alt="qoute" className='absolute w-16 -top-6 -left-6'/>
            </div>
            <div className='flex flex-row items-center gap-4'>
                <img src={props.img} alt="review" className='w-10 sm:w-14 aspect-square'/>
                <div className='text-left'>
                    <h1 className='inter-700 text-red1x  sm:text-lg'>{props.user}</h1>
                    <p className='inter-400 text-sm'>{props.job}</p>
                </div>
            </div>
        </section>
        <section className='col-span-1 h-full relative group'>
            <video src={props.video} className='min-h-full mx-auto'></video>
